...
async function fetchBatchChunk(titles: string[]): Promise<void> {
  ...
  for (const page of Object.values(pages)) {
    ...
    if (/\b(journal|film|review|account|exhibition|magazine|inc|presentations|journals|franchise|ministerial|members|microbial|oxford|museum|channel|institute|philosopher|prize|professor|department|professional|departments|singer|developer|version|publisher|documentary|book|volume|organization|organisation|lecture|course|journal|club|monographic|website|programme|university|committee|games|library|essay|textbook|philosopher|forum|list|learned|association|monograph|platform|magazine|project|band|museum|publication|initiative|author|ministry|degree|show|articles|department|channel|podcast|album)\b/i.test(firstSentence)) continue;
